...
const DeliveryTable = ({ deliveries, onAction }) => {
          ...
          deliveries.map((delivery) => (
            ...
            >
              ...
              <div className="mb-2 font-bold">
                ...
                <span className="font-normal block break-words">{delivery.address}</span>
              </div>
              <div className="mb-2 font-bold">
                <span>Date/Heure : </span>
                <span className="font-normal">{delivery.date} {delivery.time}</span>
              </div>
              <div className="mb-2 font-bold">
                <span>Statut de la livraison : </span>
                <span className="font-normal">{delivery.status}</span>
              </div>
        ...
        <table className="min-w-full bg-white border border-gray-200 rounded-lg shadow">
          <thead>
            <tr>
              ...
              <th className="px-4 py-2 border-b font-semibold text-gray-700 bg-gray-100 text-center">Adresse de livraison</th>
              <th className="px-4 py-2 border-b font-semibold text-gray-700 bg-gray-100 text-center">Date/Heure</th>
            ...
            {deliveries && deliveries.length > 0 ? (
              deliveries.map((delivery) => (
                <tr key={delivery.id} className="hover:bg-gray-50 relative">
                  ...
                  <td className="px-4 py-4 border-b text-center align-middle font-normal text-lg">{delivery.date}<br/>{delivery.time}</td>
