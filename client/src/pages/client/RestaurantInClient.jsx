  ...
  useEffect(() => {
    const fetchRestaurantData = async () => {
      ...
      try {
        ...
        const restaurantResponse = await axios.get(`http://localhost:3005/api/restaurants/getRestaurant/${id}`, {
        ...
        // Fetch featured items (e.g., from menus or items related to this restaurant)
        const itemsResponse = await axios.get(`http://localhost:3005/api/menuItem/getItemMenu/${id}`, {
          headers: { Authorization: `Bearer ${localStorage.getItem('token') || ''}` }
        });
        const itemsData = itemsResponse.data.items || itemsResponse.data; // Adjust based on API
        console.log('Featured Items:', itemsData);
        setFeaturedItems(Array.isArray(itemsData) ? itemsData : []);
      } catch (error) {
        console.error('Erreur lors du chargement des données:', error);
        setRestaurant({}); // Proceed with empty object if not found
        setFeaturedItems([]); // Clear featured items on error
      } finally {
        setIsLoading(false);
      }
    };
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div className="mb-10">
        ...
        <p className="mt-3 text-orange-500 font-semibold">
          ⭐ {restaurant.note || restaurant.rating || 'N/A'} / 5
        </p>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-6">
        {featuredItems.length > 0 ? (
          ...
          <p className="col-span-full text-center text-gray-500">No featured items available.</p>
        )}
      </div>
    </div>
  );
